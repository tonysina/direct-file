...
class MinimumSpec extends AnyFunSpec with BeforeAndAfter:
  ...
  describe("Minimum") {
    describe("can use integers") {
      it("finds the minimum in a collection") {
        ...
        assert(fact.get(0) == Result.Complete(10))
      }

      it(
        "finds the minimum in a collection and returns incomplete if not all items are complete"
      ) {
        val fact = factGraph(Path("/intTest"))(0).get
    ...
    describe("can use dollars") {
      it("finds the minimum in a collection") {
        ...
        assert(fact.get(0) == Result.Complete(Dollar(30)))
      }
    ...
    describe("can use day") {
      it("finds the minimum in a collection") {
        ...
        assert(fact.get(0) == Result.Complete(Day("2022-03-02")))
      }
    ...
    describe("can use rational") {
      it("finds the minimum in a collection") {
        ...
        assert(fact.get(0) == Result.Complete(Rational("1/3")))
      }
